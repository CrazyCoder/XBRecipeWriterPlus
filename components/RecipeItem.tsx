      ...
      <Pressable style={getStyle()} onLongPress={() => setShowDialog(true)} onPressIn={() => setPressed(true)} onPress={() => onPress()} onPressOut={() => setPressed(false)} >
        <YStack paddingHorizontal="$2" paddingVertical="$1" alignItems='center'>
          ...
          <XStack flex={1} justifyContent='space-evenly' width="100%" flexDirection='row'>
            <YStack >
              <Circle size="$7" borderColor="#ffa592" borderWidth={1}><Text fontSize={30} fontWeight={200}>{"1:" + props.recipe.getRatio()}</Text></Circle>
              <Text alignSelf='center'>Ratio</Text>
            </YStack>
            <YStack>
              <XStack flex={1} alignItems='center' justifyContent='center' borderColor="#ffa592" borderWidth={2}><Text padding="$2" fontSize={40} fontWeight={700}>{props.recipe.getTotalVolume() + " ml"}</Text></XStack>
              <Text alignSelf='center'  >Volume</Text>
            </YStack>
            <YStack >
              <Circle size="$7" borderColor="#ffa592" borderWidth={1}><Text fontSize={30} fontWeight={200}>{props.recipe.grindSize}</Text></Circle>
